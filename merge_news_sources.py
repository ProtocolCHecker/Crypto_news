...
def fetch_depinhub_news():
    url = 'https://depinhub.io/news/feed?_rsc=nedfo'
    headers = {
        'RSC': '1',
        'sec-ch-ua-platform': '"macOS"',
        'Referer': 'https://depinhub.io/news',
        'sec-ch-ua': '"Brave";v="129", "Not=A?Brand";v="8", "Chromium";v="129"',
        'sec-ch-ua-mobile': '?0',
        'Next-Router-Prefetch': '1',
        'Next-Router-State-Tree': '%5B%22%22%2C%7B%22children%22%3A%5B%22(app)%22%2C%7B%22children%22%3A%5B%22news%22%2C%7B%22children%22%3A%5B%22__PAGE__%22%2C%7B%7D%2C%22%2Fnews%22%2C%22refresh%22%5D%7D%5D%7D%5D%7D2Cnull2Cnull2Ctrue5D',
        'Next-Url': '/news',
    ...
    for item in items:
        ...
        created_at = date_obj.replace(minute=0, second=0).strftime('%Y-%m-%d %H:%M:%S')

        news_data.append({
            "News Paper": "Depinhub",
            "Created At": created_at,
            "Concatenated Text": f"{item.find('title').text}: {item.find('description').text}"
        })

    return pd.DataFrame(news_data, columns=["News Paper", "Created At", "Concatenated Text"])
...
def fetch_and_sort_news():
    ...
    # Fetch and add Depinhub news
    depinhub_news_df = fetch_depinhub_news()
    all_news_df = pd.concat([all_news_df, depinhub_news_df], ignore_index=True)
